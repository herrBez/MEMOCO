...
void printPBM(const int size, Point * a, int N){
	...
	unsigned char * img = new unsigned char[byte_num*byte_num];
	
	for(int i = 0; i < byte_num*byte_num; i++){
		img[i] = 0;
	}
	/*for(int i = 0; i < N; i++){
		int index = (int) (rint(a[i].x) * size + rint(a[i].y)); //real index
		int byte_id = index/8; // identify the corresponding byte
		int byte_offset = index % 8; //identify the offset from bit 0 to the correct bit
		img[byte_id] |= 1 << byte_offset;
	...
	ofs << byte_num << " " << byte_num << endl;
	for(int i = 0; i < byte_num; i++){
		for(int j = 0; j < byte_num; j++){
			cout << (unsigned char) img[i*byte_num+ j];
...
void printPBM(const int size, Point * a, int N){
	...
	for(int i = 0; i < size; i++){
		for(int j = 0; j < size; j++){
			ofs << (unsigned char)img[i*size + j];
		}
		ofs << endl;
	}
	ofs.close();
